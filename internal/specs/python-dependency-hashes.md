...
- `requirements/src/dev.in` is the human-edited development/lint input and
  includes `base.in`.
- `requirements/src/tools.in` is the human-edited lock-regeneration tooling
  input.
- `requirements/lock/base.txt`, `requirements/lock/dev.txt`, and
  `requirements/lock/tools.txt` are generated with hashes and must not be edited
  by hand.
...
`requirements/lock/dev.txt` before running pylint.
